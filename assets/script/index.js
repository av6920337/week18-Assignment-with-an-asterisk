...
function addLocalStorage(){
    localStorage.setItem('heroes', JSON.stringify(arrheroes));
}
addLocalStorage();
...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    arrheroes.forEach((hero, index) => {
        main.innerHTML += `
        <section class='sectionHero'>
            <h2 class='title'>${hero.name}</h2>
            <div class='blockForInfo'>
                <p class="item">
                    <span class="bold">Вселенная:</span> ${hero.universe}</p>
                <p class="item">
                    <span class="bold">Альтер эго:</span> ${hero.alterego}</p>
                <p class="item">
                    <span class="bold">Род деятельности:</span> ${hero.occupation}</p>
                <p class="item">
                    <span class="bold">Друзья:</span> ${hero.friends}</p>
                <p class="item">
                    <span class="bold">Суперсилы:</span> ${hero.superpowers}</p>
                <img src='${hero.url}' alt='${hero.name} class='item'>
                <p class='infoText'>${hero.info}</p>
            </div>
            <div class="star-rating" data-hero-index="${index}">
    ...
    initializeStarRatings(arrheroes);
    });
})

function initializeStarRatings(arrheroes) {
    const starRatings = document.querySelectorAll('.star-rating');
    starRatings.forEach(starRating => {
        const stars = starRating.querySelectorAll('.star');
        
        const heroIndex = starRating.getAttribute('data-hero-index');
        
        // Обрабатываем клики по звездочкам
        stars.forEach(star => {
            star.addEventListener('click', (e) => {
                const value = e.target.getAttribute('data-value');
                arrheroes[heroIndex].rating = value;
        ...
        // Если рейтинг для этого героя уже установлен, обновляем отображение звездочек
        if (arrheroes[heroIndex].rating) {
            updateStars(stars, arrheroes[heroIndex].rating);
        }
    });
}
